        ...
        static void Main(string[] args)
        {
            while (true)
            {
                ...
                new TimeSpan(14, 55, 0)
                    ...
                    var auditoriumUsage = auditoriums.ToDictionary(a => a.Id, a => 0);
                    ...
                    foreach (var subject in sortedSubjects)
                    {
                        ...
                        var slotsOrdered = availableSlots
                            ...
                            .OrderBy(x => x.LessonsOnDay <= 2)
                            .ThenBy(x => x.PreviousLessonEnd.HasValue ? (x.Slot.Start - x.PreviousLessonEnd.Value).TotalHours : double.MaxValue)
                            .ThenBy(x => x.NextLessonStart.HasValue ? (x.NextLessonStart.Value - x.Slot.End).TotalHours : double.MaxValue)
                            .ThenBy(x => x.Slot.Date)
                            .ThenBy(x => x.Slot.Start)
                            .Select(x => x.Slot)
                            .ToList();
